...
if (!$con) {
 die("Connection failed: " . mysqli_connect_error());
	 ...
	 else if(isset($_POST['but_submit3']))
{    	$user_id=$_SESSION['user_id'];
		...
		$sql = "INSERT INTO reviews(user_id,title,name,description,stars,link,category) VALUES ('$user_id','$title','$name','$description','$stars','$link','$category')"; 
...
$search_value=$_SESSION['uname'];
 if(!($search_value == null or $search_value == '')){	
		$sql="SELECT name,description,title,stars,fullname,link,category FROM reviews r join users u on r.user_id=u.user_id where u.email like '%$search_value%'";		
        $res=$con->query($sql);
        while($row=$res->fetch_assoc()){		
			//echo ' <div class="w3-container w3-card w3-white w3-round w3-margin"><br> ';
        echo '<u><h2><b><em>'.$row["title"].'</h2></b></em></u>';
